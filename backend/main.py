...
def download_project_zip(project_id: str, db: Session = Depends(get_db)):
    ...
    with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zip_file:
        ...
        # 1. Ensure directory structure exists (even if empty)
        zip_file.writestr("datasets/train/images/.gitkeep", "")
        zip_file.writestr("datasets/train/labels/.gitkeep", "")
        zip_file.writestr("datasets/val/images/.gitkeep", "")
        zip_file.writestr("datasets/val/labels/.gitkeep", "")
        
        # 2. Add images and labels
        for img in images:
